...
export function ProfilePage({ onNavigate, currentPath }: ProfilePageProps) {
  ...
  const surfaceClass =
    'rounded-[28px] border border-slate-200/90 bg-white/95 shadow-sm dark:border-slate-800 dark:bg-slate-900/95';
      ...
      <div className="space-y-8 lg:col-span-8">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {metrics.map((metric) => {
            ...
            return (
              <div className={`flex items-center gap-4 p-6 transition hover:-translate-y-0.5 hover:shadow-md ${surfaceClass}`} key={metric.label}>
  ...
  const teacherProfileSignals = [
    ...
    Boolean(roleSubtitle && roleSubtitle !== 'Instructor profile'),
    teacherCourses.length > 0,
    teacherSessions.length > 0,
  ];
  const teacherProfileStrength = Math.max(
    35,
    Math.min(100, Math.round((teacherProfileSignals.filter(Boolean).length / teacherProfileSignals.length) * 100)),
  );
  const teacherProfileHint =
    teacherProfileStrength >= 90
      ? 'Excellent profile quality. Keep your information up to date.'
      : teacherProfileStrength >= 70
  ...
  const teacherLocation = profile?.location || 'Location not set';
  const teacherBio =
    profile?.bio ||
  ...
  const teacherProfileBody =
    role === 'teacher' ? (
      <div className="mx-auto w-full max-w-5xl space-y-8">
        <section className={`p-4 ${surfaceClass}`}>
          ...
          <div className="flex flex-wrap items-center gap-6">
            ...
            <div className="ml-auto flex shrink-0 flex-col gap-2" style={{ width: '260px', maxWidth: '100%' }}>
              <button
                ...
                disabled={!teacherPublicProfilePath}
                onClick={() => {
                  if (teacherPublicProfilePath) {
                    onNavigate(teacherPublicProfilePath);
                  }
                }}
                type="button"
              >
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <aside className="space-y-6">
            <div className="rounded-xl border border-[#1152d4]/20 bg-[#1152d4]/5 p-6 dark:bg-[#1152d4]/10">
              ...
              <div className="mb-4 h-2.5 w-full rounded-full bg-slate-200 dark:bg-slate-800">
                <div className="h-2.5 rounded-full bg-[#1152d4]" style={{ width: `${teacherProfileStrength}%` }} />
              </div>
              <p className="mb-2 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
                {teacherProfileStrength}% complete
              </p>
            ...
            <div className={`rounded-xl p-6 ${surfaceClass}`}>
  ...
  if (role === 'teacher') {
    ...
    return (
      ...
      >
        {teacherProfileBody || profileBody}
      </TeacherSpaceShell>
    );
  }
