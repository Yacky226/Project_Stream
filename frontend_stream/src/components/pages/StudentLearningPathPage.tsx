...
import { ArrowRight, Award, BookOpen, Lock, PlayCircle, Route, Sparkles, Trophy } from 'lucide-react';
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="flex flex-col gap-6 md:flex-row">
                <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-slate-100 md:w-1/3">
                  ...
                  <ImageWithFallback
                    src={details?.coverImage || FALLBACK_COVER}
                    alt={currentModule?.title || course.title}
                  ...
                  <div>
                    <div className="mb-2 flex items-center gap-2 text-xs">
                      ...
                      <span className="font-medium text-slate-500">
                        {formatMinutes(currentModule?.minutes || 0)} remaining
                      </span>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <button
                      ...
                      onClick={() =>
                        onNavigate(
                          nextSession?.isLive
                            ? `/courses/${course.id}/live/${nextSession.id}`
                            : `/courses/${course.id}`,
                        )
                      }
                    ...
                    {index === 0 && completedModules.length > 0 ? (
                      <div className="relative pl-4">
                        ...
                        <div className="text-sm font-medium text-slate-500">
                          Milestone unlocked: {completedModules[0].title} badge
                        </div>
                      </div>
                    ) : null}
                  </div>
                    ...
                    <div className="text-center">
                      <p
                        className={`text-sm font-bold ${
                          progress >= 100 ? 'text-amber-700 dark:text-amber-300' : 'text-slate-400'
                        }`}
                      >
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-6 dark:border-slate-800">
                  <div>
                    ...
                    <p className="text-sm font-bold">
                      {formatDate(nextSession?.startAt || details?.scheduledAt || course.scheduledAt)}
                    </p>
                  </div>
                  <div>
                    <p className="mb-1 text-xs text-slate-500">Time Invested</p>
                    <p className="text-sm font-bold">{investedHours.toFixed(1)} Hours</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold">
                      {completedModules.length > 0 ? 'Fast Learner' : 'Momentum Builder'}
                    </p>
                    <p className="text-xs text-slate-500">
                      {completedModules.length > 0
                        ? `Finished ${completedModules[0].title}.`
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold">
                      {progress >= 60 ? 'Insight Master' : 'Insight Loading'}
                    </p>
                    <p className="text-xs text-slate-500">
                      {progress >= 60
                        ? `You crossed ${Math.round(progress)}% of the path.`
            ...
            <div className="rounded-2xl border border-[#1152d4]/10 bg-[#1152d4]/5 p-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{mentorSpeciality}</p>
                </div>
              </div>
