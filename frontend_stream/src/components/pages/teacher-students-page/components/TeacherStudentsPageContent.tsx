...
import {
  formatTeacherEnrollmentDate,
  normalizeTeacherEnrollmentStatus,
} from '../teacherStudents.utils';
import { Alert, AlertDescription } from '../../../ui/alert';

interface TeacherStudentsPageContentProps {
  data: TeacherStudentsPageData;
}

export function TeacherStudentsPageContent({ data }: TeacherStudentsPageContentProps) {
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-950">Students</h1>
          <p className="mt-2 text-sm text-slate-500">Liste des etudiants inscrits dans vos cours.</p>
        </div>
        ...
        >
          <RefreshCcw className={`h-4 w-4 ${data.isFetching ? 'animate-spin' : ''}`} />
          Refresh
        </button>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-bold text-slate-950">{data.summary.uniqueStudentsCount}</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">
            Active Enrollments
          </p>
          <p className="mt-2 text-3xl font-bold text-slate-950">{data.summary.activeCount}</p>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">My Courses</p>
          <p className="mt-2 text-3xl font-bold text-slate-950">{data.summary.coursesCount}</p>
        </article>
      </section>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-200">
                {data.filteredRows.map((row) => {
                  const status = normalizeTeacherEnrollmentStatus(row.status);
                  return (
                    <tr key={row.enrollmentId} className="hover:bg-slate-50">
                      ...
                      <td className="px-2 py-4 text-sm text-slate-600">
                        {formatTeacherEnrollmentDate(row.enrolledAt)}
                      </td>
                    </tr>
                  );
                })}
